 ...
 */
class Test4ScNewestReportView21 : View {
    ...
    private fun drawSpeed(canvas: Canvas) {
        ...
        list.forEachIndexed { index, string ->
            ...
            val path = Path()
            path.moveTo(speedWidth + 16f.dp2px(), textY - 4f.dp2px())
            path.lineTo(width.toFloat(), textY - 4f.dp2px())
            canvas.drawPath(path, imaginaryLinePaint)

            if (index == 4) {
                canvas.drawText("总里程", width / 2.6f - measureTextWidth(speedPaint, "总里程") / 2, textY + 20f.dp2px(), speedPaint)
                canvas.drawText("使用SC行程", width / 1.5f - measureTextWidth(speedPaint, "使用SC行程") / 2, textY + 20f.dp2px(), speedPaint)
            }
        }
    }
    ...
    fun setSchedule(height1: Float, height2: Float, text: String) {
