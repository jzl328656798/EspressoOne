 ...
 */
class Test4ScNewestReportView2 : View {
    ...
    private fun drawSpeedYLine(canvas: Canvas) {
        ...
        drawHistogramLine(canvas)
    }

    private fun drawHistogram(canvas: Canvas, y: Float) {
        val totalDistanceRectF = RectF()
        totalDistanceRectF.set(width / 2.6f - 5f.dp2px(), 200f, width / 2.6f + 5f.dp2px(), y)
        ...
        scDistanceRectF.set(width / 1.5f - 5f.dp2px(), 350f, width / 1.5f + 5f.dp2px(), y)
    ...
    private fun drawHistogramLine(canvas: Canvas) {
        histogramLinePaint.color = context.resources.getColor(R.color.color_B5A36A, null)
        canvas.drawLine(width / 2.6f - 7f.dp2px(), 200f, width / 2.6f + 7f.dp2px(), 200f, histogramLinePaint)

        val totalDistanceRectF = RectF()
        totalDistanceRectF.set(width / 2.6f - 13f.dp2px(), 200f - 19f.dp2px(), width / 2.6f + 13f.dp2px(), 200f - 3f.dp2px())
        canvas.drawRect(totalDistanceRectF, histogramLinePaint)
        drawHistogramText(canvas, "88", totalDistanceRectF)

        histogramLinePaint.color = context.resources.getColor(R.color.color_0FEEA2, null)
        canvas.drawLine(width / 1.5f - 7f.dp2px(), 350f, width / 1.5f + 7f.dp2px(), 350f, histogramLinePaint)

        val scDistanceRectF = RectF()
        scDistanceRectF.set(width / 1.5f - 13f.dp2px(), 350f - 19f.dp2px(), width / 1.5f + 13f.dp2px(), 350f - 3f.dp2px())
        canvas.drawRect(scDistanceRectF, histogramLinePaint)
        drawHistogramText(canvas, "76", scDistanceRectF)
    }
